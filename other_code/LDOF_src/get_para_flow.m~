function para=get_para_flow(I,video_dir)
[p,q,~]=size(I);
para.p=p;
para.q=q;
...
para.level = ceil((log(10)-log(para.q))/log(para.downsampling));            % levels of pyramid.:we need 70 for very large displacements
...
para.cellsize=4;  %half size of a HOG descriptor
...
para.max_dist=200;  %maximum allowed eu