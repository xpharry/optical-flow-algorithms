...
savepath = '/home/peng/dataset/farnback_flow'
...
if __name__ == '__main__':
    ...
    fid = open(savepath + '/errorfile.txt', 'w')

    for idx in range(len(files)):
        video = files[idx]
        frames = os.listdir(datapath + '/' + video)
        if len(frames) > 1:
            for i in range(len(frames)):
                gt = read_flow_file(datapath + '/' + video + '/' + frames[i] + '/1_2.flo')
                im1 = cv2.imread(datapath + '/' + video + '/' + frames[i] + '/1.png')
                im2 = cv2.imread(datapath + '/' + video + '/' + frames[i] + '/2.png')
                if im1.shape[2] == 1:
                    continue
                # prvs = cv2.cvtColor(im1, cv2.COLOR_BGR2GRAY)
                # next = cv2.cvtColor(im2, cv2.COLOR_BGR2GRAY)
                # flow = cv2.calcOpticalFlowFarneback(prvs, next, None, 0.5, 3, 15, 3, 5, 1.2, 0)
                tu, tv = read_groud_truth(gt)
                ...
                print(datapath + '/' + video + '/' + frames[i])
                print('\nAAE {:3.3f} average EPE {:f} \n'.format(aae, aepe))
                # fid.write(datapath + '/' + video + '/' + frames[i] + ':')
                fid.write('{:3.3f} {:f}\n'.format(aae, aepe))
    fid.close()
